...
import { carriagesLoadedSuccess,  ordersLoadedSuccess, rideLoadedByIdSuccess, routeLoadedByIdSuccess, routesLoadedSuccess, searchLoadedSuccess, stationsLoadedSuccess, usersLoadedSuccess } from './trips.actions';

const inititalTripState: TripsState = { stations: [], routes: [], carriages: [], orders: [], rides: [], users: [], searchResponses: [] };

export const tripsReducer = createReducer(
  inititalTripState,
  on(searchLoadedSuccess, (state, { search }): TripsState => { return { ...state, searchResponses: [...state.searchResponses, search] } }),
  ...
  on(rideLoadedByIdSuccess, (state, { ride }): TripsState => { return { ...state, rides: [...state.rides, ride] } }),
);
